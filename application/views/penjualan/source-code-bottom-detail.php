...
// Script untuk memindahkan value kode member
function updateKodeMember() {
  var input_kode_member = document.getElementById("input_kode_member").value;
  document.getElementById("kode_member").value = input_kode_member;
}
// END: Script untuk memindahkan value kode member
...
function ambilDataProduk(id_produk) {
    $.ajax({
      type: 'POST',
      data: 'id='+id_produk,
      url: '<?= base_url()."penjualan/ambilDataProduk/" ?>'+id_produk,
      dataType: 'json',
    ...
    var cek_eksis_array = array_inputan_kode_produk.includes(inputan_kode_produk);
      if (cek_eksis_array == true) {
        $.ajax({
          ...
          url: '<?= base_url()."penjualan/ambilDataProduk2/" ?>'+inputan_kode_produk,
          dataType: 'json',
          success: function (hasil) {
...
// Script untuk menambahkan titik saat menginput angka di Modal Form
  var tambah_nominal = document.getElementById('uang_diterima');
